...
class Redis_Cluster_Test extends Redis_Test {
    ...
    protected function newInstance() {
        return new RedisCluster(NULL, self::$_arr_node_map, 1, 1, true, self::AUTH);
    }
    ...
    public function testSession()
    {
        ...
        @ini_set('session.save_path', implode('&', array_map(function ($seed) {
            return 'seed[]=' . $seed;
        }, self::$_arr_node_map)) . '&failover=error' . (self::AUTH ? '&auth=' . self::AUTH : ''));
        if (!@session_start()) {
    ...
    public function testSlotCache() {
        ...
        $pong = 0;
        for ($i = 0; $i < 10; $i++) {
            $obj_rc = new RedisCluster(NULL, self::$_arr_node_map, 1, 1, true, self::AUTH);
